...
async def on_message(message: discord.Message):
    ...
    # 2) Intelligent responses for online members (prioritize active users)
        ...
        try:
            # Check if user is online (for Member objects only)
            user_online = True  # Default to true for User objects
            if hasattr(message.author, 'status'):
                user_online = message.author.status != discord.Status.offline
